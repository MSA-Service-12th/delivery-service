package com.loopang.deliveryservice.domain.event.payload;

import com.loopang.deliveryservice.domain.entity.Delivery;
import com.loopang.deliveryservice.domain.vo.delivery.Destination;
import com.loopang.deliveryservice.domain.vo.delivery.Origin;

import java.util.UUID;

// 배송 -> 주문 방향으로 이벤트 발송
// 배송 생성, 배송완료 및 취소 시 수신
public record DeliveryUpdatePayload(
		UUID orderId,
		UUID departureId,   // 출발지ID (공급업체 허브ID)
		UUID destinationId, // 목적지ID (수령업체 허브ID 또는 수령업체ID)
		UUID deliveryId,    // 생성된 배송ID
		String deliveryStatus // 배송상태 (Enum 매핑 전 원본 문자열)
) {
	public static DeliveryUpdatePayload from(Delivery delivery) {
		Origin origin = delivery.getOrigin();
		Destination destination = delivery.getDestination();

		return new DeliveryUpdatePayload(
				delivery.getOrderId(),
				origin.getDepartureHubId(),
				destination.getReceiptHubId(),
				delivery.getDeliveryId(),
				delivery.getStatus().name()
		);
	}
}
