...
public class DeliveryQueryService {
    ...
    // 단건 조회
    public DeliveryResponseDto getDelivery(UUID deliveryId, UUID userId, String userRole) {
        ...
        if (type == UserType.DELIVERY) {
            if (!userId.equals(delivery.getHubCourierId()) && !userId.equals(delivery.getCompanyCourierId())) {
    ...
    // 목록 조회
    public Page<DeliveryResponseDto> getDeliveries(DeliveryQueryCondition condition, Pageable pageable, UUID userId, String userRole) {
        UserType type = UserType.from(userRole);
        
        // [권한 규칙 적용] 배송 담당자는 자신의 담당 배송만 조회 가능하도록 필터링 강제
        if (type == UserType.DELIVERY) {
            condition.setSearchCourierId(userId);
        }
